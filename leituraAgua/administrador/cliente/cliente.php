...
$sql="SELECT * FROM usuario WHERE categoria='Cliente'";
$res=$conn->query($sql);
    ...
    while($row=$res->fetch_object()){
           ...
           $row->estate? $estatebloquear='Tem a certeza que deseja desbloquear?':$estatebloquear='Tem a certeza que deseja bloquear?';
            print"<tr>";
                    ...
                    print"<td>$estado</td>";
                    print"<td><button onclick=\" if(confirm('{$estatebloquear}')){location.href='?page=operacliente&acao=bloquear&id=".$row->id."';} else{false;}\"  class='{$colorbtn}' >$estatebtn</button></td>";
                    print"<td><button onclick=\"location.href='?page=editarcliente&id=".$row->id."';\" class='btn btn-primary'>Editar</button></td>";
